...
//        위에 리스트 안에는 3개의 테이블을 조인해서 가져온 장바구니id,회원id,회원이름,상품id,상품이름,장바구니수량,상품단가,(상품*수량)총금액이 담겨있다
    ...
    @PostMapping("/update")
    public String update(@RequestParam int[] productQu, @RequestParam long[] productId, HttpSession session) {
        //RequestParam으로 productQu와 productId를 배열로 받는 이유는 장바구니에 상품이 하나만 있는게 아니니까 여러 상품을 받아서 수정하기 위해서이다
        String loginEmail = (String) session.getAttribute("loginEmail");
        ...
        for (int i = 0; i < productId.length; i++) {
            ...
            cartDTO.setMemberId(memberDTO.getId());
            cartDTO.setProductQu(productQu[i]);
            cartDTO.setProductId(productId[i]);
            // 회원아디,상품아디,바꾼수량이 담겨있다
